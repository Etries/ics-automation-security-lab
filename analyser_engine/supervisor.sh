...
while true; do
  ...
  timeout "$CAPTURE_DURATION" tcpdump -i "$INTERFACE" -s "$PCAP_SNAPLEN" -C "$PCAP_SIZE_LIMIT_MB" -w "$PCAP" vlan and port 502
  sudo tcpdump -i eth0 -c 5 -w /home/kali/Desktop/Thesis_framework/ics-automation-security-lab/analyser_engine/runtime/pcaps/test_segment_20250718_082444.pcap
  if [[ ! -s "$PCAP" ]]; then
