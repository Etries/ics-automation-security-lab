...
class Detector:
    ...
    def _check_polling_sequence(self, e):
        ...
        # ---- CHANGED: key polling by src->dst (not uid) so it survives per-PDU TCPs
        ...
        if not active:
            self.state.reset_polling_state(conn_key)  # CHANGED: use conn_key
        ...
        if current_func == expected_func:
            ...
            # ---- CHANGED: alert on order mismatch, then try to resync for future checks
    ...
    def analyze_log(self, log_file, vlan_map_file):
        ...
        for e in events:
            ...
            if e['function_code'] in [5, 6, 15, 16]:
                # ---- CHANGED: reset polling by src->dst key (not uid) to match checker
