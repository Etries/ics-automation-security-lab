...
class Detector:
    def __init__(self, policy_file, addr_map_file):
        ...
        # ---- Polling tuning (to reduce false positives) ----
        self.polling_idle          = float(self.polling_rules.get('max_idle_time', 10.0))
        self.polling_strict_start  = bool(self.polling_rules.get('strict_start', False))
        self.polling_resync_grace  = int(self.polling_rules.get('resync_grace', 1))  # free resyncs
        self.polling_tempo_epsilon = float(self.polling_rules.get('tempo_epsilon', 0.02))  # 20ms slack

        # NEW: very small knobs (optional in policy)
        self.polling_preferred     = self.polling_rules.get('preferred_sequence')  # name or None
        self.polling_tempo_grace   = int(self.polling_rules.get('tempo_miss_grace', 2))
    ...
    def _load_vlan_map(self, vlan_file):
        uid_to_vlan, pair_to_vlan = {}, {}
        router_macs = set(m.lower() for m in self.policy.get('network', {}).get('router_macs', []))
        try:
            with open(vlan_file) as f:
                for line in f:
                    ...
                    if len(parts) == 2 and parts[1].isdigit():
                        ...
                        continue
                    # FORMAT 2: "<src>\t<dst>\t<vlanid>"
                    if len(parts) >= 3 and parts[2].isdigit() and '.' in parts[0] and '.' in parts[1]:
                        pair_to_vlan[f"{parts[0]}-{parts[1]}"] = int(parts[2])
                        continue
                    # FORMAT 3: "<src>\<vlan>\<mac>"
                    if len(parts) >= 3 and parts[1].isdigit() and '.' in parts[0]:
                        src, vlan, mac = parts[0], int(parts[1]), parts[2].lower()
                        if mac in router_macs:
                            continue  # ignore router-emitted frames
                        pair_to_vlan[f"{src}-*"] = vlan
                    # FORMAT 3 (no mac): "<src>\<vlan>"
                    elif len(parts) >= 2 and parts[1].isdigit() and '.' in parts[0]:
                        pair_to_vlan[f"{parts[0]}-*"] = int(parts[1])
        except Exception as ex:
            print(f"[vlan] parse error: {ex}")
            pass
        print(f"[vlan] uid_to_vlan entries: {len(uid_to_vlan)}")
        print(f"[vlan] pair_to_vlan entries: {len(pair_to_vlan)}")
        for k in list(pair_to_vlan.keys())[:5]:
            print(f"[vlan] {k} -> {pair_to_vlan[k]}")
    ...
    def _check_vlan_violation(self, e, uid_to_vlan, pair_to_vlan):
        ...
        if e['uid'] in uid_to_vlan:
            ...
            actual_vlan = pair_to_vlan.get(f"{e['src']}-{e['dst']}") or pair_to_vlan.get(f"{e['src']}-*")
    ...
    # ---- Simpler + calmer polling sequence ----
    def _check_polling_sequence(self, e):
        if not (e.get('is_request') and e['function_code'] in [1,2,3,4]):
            return None

        current_time, current_func = e['ts'], e['func_text']
        conn_key = f"{e['src']}->{e['dst']}"
        key = f"{conn_key}_polling"
        ...
        # Helper: set state to a given sequence at index (warm-up: 0 steps, 0 resyncs)
        def set_state(seq_name, idx):
            self.state.polling_state[key] = {
                'last_time': current_time,
                'index': idx,
                'active_sequence_name': seq_name,
                'steps': 0,          # how many successful transitions since sync
                'resyncs': 0,        # order mismatches forgiven so far
                'tempo_misses': 0,   # tempo misses in a row
            }

        # If idle gap was large, silently reset (no alert)
        if st and (current_time - st['last_time'] > self.polling_idle):
            st = None
            try: del self.state.polling_state[key]
            except KeyError: pass

        # No state yet → choose starting position
        if not st:
            # Prefer preferred_sequence if it contains current_func
            if self.polling_preferred:
                pref = next((s for s in sequences if s['name'] == self.polling_preferred), None)
                if pref and current_func in pref['function_sequence']:
                    set_state(pref['name'], pref['function_sequence'].index(current_func))
                    return None

            if self.polling_strict_start:
                starters = [s for s in sequences if s['function_sequence'][0] == current_func]
                if starters:
                    set_state(starters[0]['name'], 0)
                    return None
                return {'rule_id': 'R016',
                        'description': f'Polling start violation: {current_func} is not a valid sequence start'}

            # non-strict: align to first matching sequence without alert
            for seq in sequences:
                if current_func in seq['function_sequence']:
                    set_state(seq['name'], seq['function_sequence'].index(current_func))
                    return None
            return {'rule_id': 'R016', 'description': f'Polling violation: {current_func} not in any defined sequence'}

        # Have state → enforce order & tempo
        ...
        # Case 1: Function is as expected → check tempo (calmly)
        if current_func == expected_func:
            if st['steps'] >= 1:
                lo = max(0.0, float(interval['min_time']) - self.polling_tempo_epsilon)
                hi = float(interval['max_time']) + self.polling_tempo_epsilon
                if not (lo <= time_diff <= hi):
                    st['tempo_misses'] += 1
                    if st['tempo_misses'] < self.polling_tempo_grace:
                        # forgive this miss and advance
                        st['last_time'] = current_time
                        st['index'] = expected_index
                        st['steps'] += 1
                        return None
                    # exceeded grace → alert + reset state for continuity
                    try: del self.state.polling_state[key]
                    except KeyError: pass
                    return {'rule_id': 'R016', 'description': f'Polling rhythm broken: {time_diff:.3f}s interval'}
            # good step, reset tempo miss counter
            st['tempo_misses'] = 0
            st['last_time'] = current_time
            st['index'] = expected_index
            st['steps'] += 1
        ...
        # Case 2: Order mismatch → try to resync without switching wildly
        if st.get('resyncs', 0) < self.polling_resync_grace:
            # 1) Try to realign within ACTIVE sequence first
            if current_func in func_seq:
                idx = func_seq.index(current_func)
                set_state(active['name'], idx)
                self.state.polling_state[key]['resyncs'] = st.get('resyncs', 0) + 1
                return None

            # 2) Then try preferred sequence from policy, if provided
            if self.polling_preferred:
                pref = next((s for s in sequences if s['name'] == self.polling_preferred), None)
                if pref and current_func in pref['function_sequence']:
                    idx = pref['function_sequence'].index(current_func)
                    set_state(pref['name'], idx)
                    self.state.polling_state[key]['resyncs'] = st.get('resyncs', 0) + 1
                    return None

            # 3) Finally fall back to first other matching sequence
            for seq in sequences:
                if seq['name'] in (active['name'], self.polling_preferred or ''):
                    continue
                if current_func in seq['function_sequence']:
                    idx = seq['function_sequence'].index(current_func)
                    set_state(seq['name'], idx)
                    self.state.polling_state[key]['resyncs'] = st.get('resyncs', 0) + 1
                    return None

            # no sequence contains this func → hard violation
            try: del self.state.polling_state[key]
            except KeyError: pass
            return {'rule_id': 'R016', 'description': f'Polling violation: {current_func} not in any defined sequence'}

        # Case 3: Exceeded grace → alert and resync to where we are
        desc = f'Polling order violation: expected {expected_func}, got {current_func}'
        # resync to any sequence that contains current_func to keep continuity
        for seq in sequences:
            if current_func in seq['function_sequence']:
                idx = seq['function_sequence'].index(current_func)
                set_state(seq['name'], idx)
                break
        return {'rule_id': 'R016', 'description': desc}
    ...
    def analyze_log(self, log_file, vlan_map_file):
        ...
        for e in events:
            ...
            if e['function_code'] in [5, 6, 15, 16]:
                # reset polling by src->dst key (to match polling checker)
                self.state.reset_polling_state(f"{e['src']}->{e['dst']}")
